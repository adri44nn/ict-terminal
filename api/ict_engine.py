...
class ICTEngine:
    ...
    def analyze_symbol(cls, symbol_key: str, candles: List[Dict[str, float]], timeframe: str = "5m") -> Dict[str, Any]:
        ...
        recent_sweeps = [s for s in sweeps if s['candle_index'] >= n - lookback]
        recent_mss = [m for m in mss_events if m['break_index'] >= n - lookback]
        recent_fvgs = [f for f in fvgs if f['candle_index'] >= n - lookback and not f['mitigated']]
        ...
        is_bull_candidate = (recent_bull_mss and bool(recent_bull_fvg))
        is_bear_candidate = (recent_bear_mss and bool(recent_bear_fvg))
        ...
        min_struct_risk = 15.0 if symbol_key == 'MNQ' else (4.0 if symbol_key == 'MES' else (2.0 if symbol_key == 'M2K' else 3.5))

        if is_bull_candidate:
            fvg = recent_bull_fvg[-1]
            temp_entry = round(fvg['top'], 2)
            prior_lows = [sl['price'] for sl in swing_lows if sl['price'] < temp_entry]
            struct_sl = prior_lows[-1] if prior_lows else (temp_entry - min_struct_risk)
            temp_sl = round(min(struct_sl, fvg['bottom'] - (min_struct_risk * 0.3)), 2)
            if (temp_entry - temp_sl) < min_struct_risk:
                temp_sl = round(temp_entry - min_struct_risk, 2)
            risk = round(max(temp_entry - temp_sl, min_struct_risk), 2)

            valid_bsl = [sh['price'] for sh in swing_highs if (sh['price'] - temp_entry) >= (1.8 * risk) and (sh['price'] - temp_entry) <= (4.5 * risk)]
            if valid_bsl:
                target_bsl = round(valid_bsl[0], 2)
            else:
                target_bsl = round(temp_entry + (2.5 * risk), 2)

            reward = round(target_bsl - temp_entry, 2)
            rr_ratio = round(reward / max(risk, 0.1), 2)

            target_hit = current_price >= target_bsl
            sl_hit = current_price <= temp_sl

            if rr_ratio >= 1.8 and not target_hit and not sl_hit:
                direction = "BULLISH"
                ...
                if time_info['is_silver_bullet']:
                    ...
                    grade_desc = f"A+ Setup: Silver Bullet Macro Window + Sweep + MSS + FVG (1:{rr_ratio} RR)"
                    setup_type = "ICT Silver Bullet Model (Bullish Buy Limit)"
                elif time_info['is_high_probability_time'] and has_sweep:
                    setup_grade = "A"
                    grade_badge = "GRADE_A"
                    grade_desc = f"A Setup: Prime Killzone Session + Full Confluence (1:{rr_ratio} RR)"
                    setup_type = "ICT 2022 Mentorship Model (Bullish Buy Limit)"
                else:
                    setup_grade = "B"
                    grade_badge = "GRADE_B"
                    grade_desc = f"B Setup: Valid Bullish FVG Retest Structure (1:{rr_ratio} RR)"
                    setup_type = "ICT Bullish Sweep + FVG Retest"
            else:
                entry_price = temp_entry
                stop_loss = temp_sl
                take_profit = target_bsl
                setup_grade = "B" if confluence_score >= 50 else "F"
                grade_badge = "GRADE_B" if confluence_score >= 50 else "GRADE_F"
                grade_desc = f"Structure Forming: Retracing into FVG ({temp_entry})" if not target_hit else "Target Already Reached"
                setup_type = f"Bullish FVG Retest Forming (1:{rr_ratio} RR)"

        elif is_bear_candidate:
            fvg = recent_bear_fvg[-1]
            temp_entry = round(fvg['bottom'], 2)
            prior_highs = [sh['price'] for sh in swing_highs if sh['price'] > temp_entry]
            struct_sl = prior_highs[-1] if prior_highs else (temp_entry + min_struct_risk)
            temp_sl = round(max(struct_sl, fvg['top'] + (min_struct_risk * 0.3)), 2)
            if (temp_sl - temp_entry) < min_struct_risk:
                temp_sl = round(temp_entry + min_struct_risk, 2)
            risk = round(max(temp_sl - temp_entry, min_struct_risk), 2)

            valid_ssl = [sl['price'] for sl in swing_lows if (temp_entry - sl['price']) >= (1.8 * risk) and (temp_entry - sl['price']) <= (4.5 * risk)]
            if valid_ssl:
                target_ssl = round(valid_ssl[0], 2)
            else:
                target_ssl = round(temp_entry - (2.5 * risk), 2)

            reward = round(temp_entry - target_ssl, 2)
            rr_ratio = round(reward / max(risk, 0.1), 2)

            target_hit = current_price <= target_ssl
            sl_hit = current_price >= temp_sl
            ...
            if rr_ratio >= 1.8 and not target_hit and not sl_hit:
                ...
                if time_info['is_silver_bullet']:
                    ...
                    setup_grade = "B"
                    grade_badge = "GRADE_B"
                    grade_desc = f"B Setup: Valid Bearish FVG Retest Structure (1:{rr_ratio} RR)"
                    setup_type = "ICT Bearish Sweep + FVG Retest"
            else:
                entry_price = temp_entry
                stop_loss = temp_sl
                take_profit = target_ssl
                ...
                grade_badge = "GRADE_B" if confluence_score >= 50 else "GRADE_F"
                grade_desc = f"Structure Forming: Retracing into FVG ({temp_entry})" if not target_hit else "Target Already Reached"
                setup_type = f"Bearish FVG Retest Forming (1:{rr_ratio} RR)"

        else:
            # Check if forming setup deserves a B grade (e.g. 2 key structural pieces formed)
            if (has_sweep and has_mss) or (has_sweep and has_fvg) or confluence_score >= 50:
                setup_grade = "B"
                grade_badge = "GRADE_B"
                grade_desc = f"B Potential: Structure Forming ({confluence_score}% Confluence Criteria Met)"
